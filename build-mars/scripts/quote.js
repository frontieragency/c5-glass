...
(function () {
  ...
  function computeRange(svc) {
    ...
    var mid = (base * mult + add + (svc.per_sqft || 0) * area) * count;
  ...
  function renderDetails() {
    ...
    if (state.mode === 'instant') {
      wrap = el('<div class="wc-q__panel"><p class="wc-q__back">&larr; Change service</p><h2 class="wc-h2">' + s.name + '</h2><div class="wc-q__fields"></div></div>');
  ...
  function renderResult() {
    ...
    if (state.mode === 'instant') {
      ...
      inner = '<p class="wc-kicker">Your estimate</p>' +
        ...
        '<p class="wc-q__sent">We&rsquo;ve sent your details to ' + cfg.business + ' — expect a call or text shortly to confirm and book a free measurement.</p>';
    } else {
    ...
    var wrap = el('<div class="wc-q__panel wc-q__result">' + inner +
      '<div class="wc-q__actions"><a class="wc-btn wc-btn--accent" href="tel:' + (cfg.phoneRaw || '') + '">Call us now</a>' +
